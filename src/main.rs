...
impl VolumeControlTarget for pulsectl::controllers::types::DeviceInfo {
    ...
    fn mute(&self, mute: bool, controller: &mut SinkController) {
        let op = controller.handler.introspect.set_sink_mute_by_name(
            self.name.as_ref().unwrap(),
            mute,
            Option::Some(Box::new(|_| {})),
        );
...
impl VolumeControlTarget for pulsectl::controllers::types::ApplicationInfo {
    ...
    fn is_default(&self, _: &mut SinkController) -> bool {
        false
    }
    fn is_muted(&self) -> bool {
        self.mute
    ...
    fn set_volume(&self, new_vol: pulse::volume::ChannelVolumes, controller: &mut SinkController) {
        let op = controller
            .handler
            .introspect
            .set_sink_input_volume(self.index, &new_vol, None);
        controller.handler.wait_for_operation(op).unwrap();
    }
}
...
struct Opt {
    ...
    #[arg(
        long,
        default_value = "audio-volume-muted",
        display_order(11),
        value_name = "ICON"
    )]
    ...
    #[arg(
        long,
        default_value = "audio-volume-high",
        display_order(11),
        value_name = "ICON"
    )]
...
fn adjust_volume(
    ...
    let (devices_to_control, write_prev_device) = {
        ...
        if running_devices.len() > 0 {
            ...
            let prev_devices = match std::fs::read_to_string(&prev_device_filename) {
                Ok(prev_device_name) => targets
                    .iter()
                    .filter(|target| prev_device_name == target.name())
                    .collect(),
            ...
            if prev_devices.len() > 0 {
                ...
                (
                    targets
                        .iter()
                        .filter(|target| target.is_default(&mut controller))
                        .collect(),
                    false,
                )
            }
    ...
    for device in devices_to_control {
        let prev_notification_id_filename =
            format!("{}_{}_{}", tmpfile_base, device.name(), "notification-id");
        ...
        prev_id = if opt.mute_toggle {
            ...
            show_notification(
                prev_id,
                if is_now_muted {
                    pulse::volume::VOLUME_MUTED.0
                } else {
                    device.volume().avg().0
                },
                &device.description(),
                if is_now_muted {
                    opt.icon_muted.as_str()
                } else {
                    opt.icon.as_str()
                },
                opt.duration,
            ...
            let old_volume = if muted && opt.increase.is_some() {
                pulse::volume::VOLUME_MUTED.0
            } else {
                device.volume().avg().0
            };
            ...
            let new_volume_raw = (old_volume as f64
                + ((pulse::volume::VOLUME_NORM.0 - pulse::volume::VOLUME_MUTED.0) as f64 * factor))
                .round() as i64;
            let new_volume = if opt.increase.is_some() {
                ...
                } else if old_volume < pulse::volume::VOLUME_NORM.0
                    && (pulse::volume::VOLUME_NORM.0 as i64) < new_volume_raw
                {
                ...
                if old_volume > pulse::volume::VOLUME_NORM.0
                    && (pulse::volume::VOLUME_NORM.0 as i64) > new_volume_raw
                {
                    pulse::volume::VOLUME_NORM.0 as i64 // snap to 100% value
                } else {
            ...
            assert!(
                new_volume as u32 <= pulse::volume::VOLUME_MAX.0,
            ...
            let mut value_steps = std::vec::Vec::from_iter((1..opt.steps + 1).map(|step_number| {
                if step_number == opt.steps {
                    ...
                    (old_volume as f64
                        + (((new_volume as f64 - old_volume as f64) / opt.steps as f64)
                            * step_number as f64))
                        .round() as u32
                }
            }));
            ...
            for (new_volume, sleep_at_the_end) in value_steps
                .into_iter()
                .enumerate()
                .map(|(i, step_value)| (step_value, i + 1 != value_steps_len))
            {
...
fn main() {
    ...
    nix::fcntl::flock(
        std::os::unix::io::AsRawFd::as_raw_fd(&lockfile),
    ...
    if opt.application {
        adjust_volume(
            controller
                .list_applications()
                .expect("Could not get list of playback applications"),
            opt,
            tmpfile_base,
            controller,
        );
...
fn show_notification(
    ...
    let percentage =
        (100.0 * new_volume as f64 / pulse::volume::VOLUME_NORM.0 as f64).round() as i32;
    let progressbar_percentage =
        (100.0 * new_volume as f64 / pulse::volume::Volume::ui_max().0 as f64).round() as i32;

    println!(
        "new: {}%, progress new: {}%",
        percentage, progressbar_percentage
    );
